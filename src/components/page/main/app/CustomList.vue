        ...
        <div class="box">
            ...
            <div class="table-area">
                <div style="text-align:right;">
                    <div class="w200">
                        <el-select v-model="menuId" placeholder="请选择需要操作的菜单" @change="menuCascaderChange">
                            <el-option v-for="(item, i) in filterTypeList" :key="i" :label="item.name" :value="item.id"></el-option>
                        </el-select>
                    </div>
            ...
            <el-dialog
                ...
                <el-table
                    ...
                    <el-table-column prop="goodsDetail" label="商品描述">
                    </el-table-column>
                    <el-table-column prop="isNew" label="是否新品">
                        <template slot-scope="scope">
                            {{scope.row.isNew == 0 ? '否' : '是'}}
                    ...
                    <el-table-column prop="label" label="特色">
                        <template slot-scope="scope">
                    ...
                    <el-table-column prop="imgUrl" label="封面图片" :show-overflow-tooltip="true">
                        <template slot-scope="scope">
                            <el-image 
                                v-if="area == 'eu'"
                                v-show="scope.row.imgUrl"
...
export default {
    ...
    methods: {
        ...
        getData() {
            getType().then(res => {
                let commonRes = res.filter(item => item.type !== 1)
                let fres = res.filter(item => item.type !== 0)
                
                let str = JSON.stringify(fres)
                let str2 = str.replace(/,"children":\[\]/g, '')
                this.filterTypeList = JSON.parse(str2)

                let fstr = JSON.stringify(commonRes)
                let fstr2 = fstr.replace(/,"children":\[\]/g, '')
                this.typeList = JSON.parse(fstr2)
            })
        },
        getGoodList() {
            let obj = {
        ...
        create() {
            if (!this.menuId) {
                this.$message.info('请先选择要操作的自定义菜单');
                return
            }
            this.operate = 'create'
            this.initData()
