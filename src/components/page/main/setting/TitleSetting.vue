        ...
        <div>
            ...
            <div class="attr-box">
                <el-tag
                    ...
                    <span v-if="!tag.editVisible">{{tag.configValue}}</span>
                    <div class="edit-input" v-show="tag.editVisible">
                        <el-input :ref="'editInput' + i" v-model="form.configValue" @blur="save(i)"></el-input>
                    </div>
                </el-tag>
                <div class="add-input" v-show="dialogVisible">
                    <el-input ref="addInput" v-model="form.configValue" @blur="save"></el-input>
                </div>
                    <el-button v-if="!(value == 'taxRate' && tags.length > 0)" class="button-new-tag" size="small" @click="showInput(value)">+</el-button>
            </div>
        </div>
...
export default {
    ...
    data() {
        return {
            ...
            form: {
                id: '',
                configKey: '', // taxRate汇率，quarter季度 0支出 1收入  
                configValue: '', 
            },
            dialogVisible: false,
        };
    ...
    methods: {
        ...
        edit(tag, type, i) {
            this.form.id = tag.id
            this.form.configValue = tag.configValue
            this.form.configKey = type
